#!/bin/bash
# Enhanced Backend deployment script with network validation and health checks
...
# Determine the API port based on environment
if [ "$ENV" == "dev" ]; then
  SERVER_PORT=8080
elif [ "$ENV" == "qa" ]; then
  SERVER_PORT=8082
...
# Verify backend API is healthy (enhanced health check)
echo "Verifying API health..."
max_attempts=30
counter=0
backend_container="pharmacyhub-backend-$ENV"

while [ $counter -lt $max_attempts ]; do
  if docker compose -f docker-compose.yml exec -T $backend_container curl -s -o /dev/null -w "%{http_code}" http://localhost:$SERVER_PORT/actuator/health | grep -q "200"; then
    echo "API is healthy!"
    break
  fi
  counter=$((counter + 1))
  if [ $counter -eq $max_attempts ]; then
    ...
    docker compose -f docker-compose.yml logs $backend_container --tail 100
    exit 1
  fi
  echo "Attempt $counter/$max_attempts: API not healthy yet..."
  sleep 10
done
